...
# ──────────────────────────────────────────────
# MODEL BUILDERS  (match training code exactly)
# ──────────────────────────────────────────────
...
# BOOT — load models
# ──────────────────────────────────────────────
with st.spinner("🧠 Loading models (first run downloads from HF Hub, then cached locally)…"):
...
clinical conversations.
    ...
    # else:
    #     st.info(
    #         "Leaderboard CSV not found at `Results/Model_Leaderboard/leaderboard.csv`. "
    #         "Add it to your project directory to display results here.",
    #         icon="📊",
    #     )
...
with tab_mri:
    ...
    if uploaded_files:
        ...
        st.info("👆 Upload one or more MRI images above to get started.", icon="ℹ️")
...
with tab_chat:
    ...
    with st.expander("⚙️ Manage this chat"):
        ...
        if c1.button("Save name"):
            active["name"] = renamed
            st.rerun()
        if c2.button("🗑️ Delete chat", disabled=len(st.session_state.chat_sessions) <= 1):
    ...
    if user_prompt:
        ...
        with st.spinner("Thinking..."):
            ...
            try:
                ...
                assistant_reply = f"⚠️ Could not reach the local Ollama model.\n\nError: {e}"
...
#     st.header("AI Assistant")
#     st.markdown(
#         "Ask me anything about Alzheimer's Disease — stages, symptoms, diagnosis, "
#         "caregiving, research, and more. Powered by a local Ollama model."
#     )

#     # Chat input FIRST — Streamlit pins it to the bottom of the viewport
#     user_prompt = st.chat_input("Ask about Alzheimer's Disease...")

#     # Then render message history in a container
...
# """
# AD-Stage-Net — Streamlit App
# Tabs: Home | MRI Analysis | AI Assistant
# """
...
#     selected_model_2 = None
#     if use_ensemble:
#         remaining = [m for m in ARCHITECTURE_OPTIONS if m != selected_model_1]
#         selected_model_2 = st.selectbox("Secondary Model", remaining)
...
#     st.divider()
#     st.caption("AD-Stage-Net v3.0 · For research use only")

# # ──────────────────────────────────────────────
# # MAIN TITLE
# # ──────────────────────────────────────────────
# st.title("AD-Stage-Net")
# st.caption("Cross-Dataset AI Classification for Alzheimer’s Disease Using Brain MRI Scans")

# # ──────────────────────────────────────────────
# # TABS
# # ──────────────────────────────────────────────
# tab_home, tab_mri, tab_chat = st.tabs([
#     "🏠 Home",
#     "🔬 MRI Analysis",
#     "💬 AI Assistant",
# ])
...
#     # --- Disclaimer banner ---
#     st.warning(
#         "**Disclaimer:** AD-Stage-Net is a research tool only and is **not** a "
#         "medical device. It is not intended to diagnose, treat, or replace professional "
#         "medical advice. Always consult a qualified healthcare provider.",
#         icon="⚠️",
#     )

#     st.header("About AD-Stage-Net")
#     st.markdown(
#         """
#         **AD-Stage-Net** is a deep learning system that classifies brain MRI scans into
#         four stages of Alzheimer's Disease (AD) severity. It was built to explore how
#         convolutional neural networks (CNNs) can support early detection research.

#         The models were trained on a curated, combined dataset sourced from Kaggle and
#         Hugging Face, improving diversity and generalizability. This app lets you upload
#         your own MRI slices and receive a real-time classification result.

#         **Links:**
#         - 📂 [GitHub Repository](https://github.com/katelynhur/AD-Stage-Net)
#         - 🤗 [Hugging Face Space](https://huggingface.co/spaces/katelynhur/AD-Stage-Net)
#         """
#     )
...
#         60–80 % of cases. It is a progressive neurological disorder that slowly
#         destroys memory and cognitive function.

#         Early detection is critical — interventions are significantly more effective
...
#     with col2:
#         st.info("🔵 **Very Mild Impairment**")
#         st.markdown("Subtle memory lapses. Slight structural changes may be detectable.")
#     with col3:
#         st.warning("🟡 **Mild Impairment**")
#         st.markdown("Noticeable memory and cognitive difficulties. Visible atrophy on MRI.")
#     with col4:
#         st.error("🔴 **Moderate Impairment**")
#         st.markdown("Significant decline in daily functioning. Pronounced brain changes.")
...
#     st.header("Model Performance")
#     st.info(
#         "📊 Performance leaderboard will be loaded here from `Results/Model_Leaderboard/leaderboard.csv` "
#         "once the full MRI analysis module is wired up in Step 2.",
#         icon="ℹ️",
#     )
...
# # ══════════════════════════════════════════════
# # TAB 2 — MRI ANALYSIS  (placeholder)
# # ══════════════════════════════════════════════
# with tab_mri:
#     st.header("MRI Analysis")
#     st.info(
#         "MRI upload, model loading, inference, and results visualization "
#         "will be built out in Step 2.",
#         icon="ℹ️",
#     )

#     # Placeholder uploader so the tab feels usable
#     uploaded_file = st.file_uploader(
#         "Upload an MRI image (JPG, JPEG, PNG)",
#         type=["jpg", "jpeg", "png"],
#     )
#     if uploaded_file:
#         st.image(uploaded_file, caption="Uploaded MRI", use_container_width=False, width=400)
#         st.info("Model inference coming in Step 2.")
...
#     st.header("AI Assistant")
#     st.markdown(
#         "Ask questions about Alzheimer's Disease — symptoms, stages, caregiving, "
#         "research, and more. Powered by a local Ollama model."
#     )
#     st.info(
#         "💬 Full chat interface with AD-focused system prompt will be wired up in Step 3.",
#         icon="ℹ️",
#     )
...
#     with st.chat_message("assistant"):
#         st.markdown(
#             "👋 Hello! I'm your Alzheimer's Disease assistant. "
#             "Ask me anything about AD stages, symptoms, caregiving strategies, or the latest research."
#         )

#     st.chat_input("Ask about Alzheimer's Disease...", disabled=True)